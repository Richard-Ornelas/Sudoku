function hide() {
  var x = document.querySelector('#regras')
...
function start() {
  ...
  $(document).ready(function () {
    for (let l = 0; l <= 8; l++) {
      ...
      for (let c = 0; c <= 8; c++) {
        let input = $(
          `<input type="text" id="c${l}${c}" value="${sudoku[l][c]}">`
        )
  ...
  function jogada_valida(sudoku, lin, col, valor) {
    if (isNaN(valor)) {
      return false
    } // Não possibilita por letras

    let linha1 = 0
    let coluna1 = 0

    // verifica em que grupo de três está, para o 'for' percorrer apenas nele
    // condição para delimitar as casas que vão ser analizadas(3x3)
    if (lin <= 2) {
    ...
    //percorrer as 3 casas da linha
    for (let i = linha1 - 2; i <= linha1; i++) {
      // Percore pela maior e menor linha do grupo que o 'valor' está.
      for (let j = coluna1 - 2; j <= coluna1; j++) {
        // Percore pela maior e manor coluna do grupo que o 'valor' está.
